...
export class BaseService {
    ...
    constructor(@Inject(String) protected apiName: string, protected http: HttpClient) {
        this.apiUrl += apiName;
    }
    ...
    mapArrayResponseToObject<T>(response: any): T {
        throw new Error('mapArrayResponseToObject must be set in specified service');
        // const mapped = response.map((responseJson: any) =>
        //   this.factory<T>(responseJson)
        // );
        // return mapped;
    ...
    skipAndTake<T>(skipTakeInput: SkipTakeInput, appendToUrl: string = '', params: string = ''): Observable<T> {
        return this.http.get(`${this.apiUrl}/Skip/${skipTakeInput.skip}/Take/${skipTakeInput.take}${appendToUrl}?${params}`).pipe(
            // retry(2),
            map((response: any) => this.mapArrayResponseToObject<T>(response)),
            catchError((error) => {
                return this.handleError(error);
